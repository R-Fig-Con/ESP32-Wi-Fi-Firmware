...
uint16_t TRAFFIC_GEN::getTimeFromGauss(){
    ...
    if( pseudo_rand>gauss_arr[0] ) delay = (uint16_t)time_interval;
    else if( pseudo_rand>gauss_arr[1] ) delay = (uint16_t)(gauss_arr[0]*time_interval);
    else if( pseudo_rand>gauss_arr[2] ) delay = (uint16_t)(gauss_arr[1]*time_interval);
    else delay = (uint16_t)(gauss_arr[2]*time_interval);
...
bool TRAFFIC_GEN::setTime(uint8_t time_mode, uint16_t waiting_time){
    if(time_mode != TRF_GEN_CONST && time_mode != TRF_GEN_GAUSS) return false;
